...
public class ContentIdOptionsSupplier implements ForeignKeyOptionsSupplier {
...
public ListingData<Option> search(String searchText, Pageable pageable, HttpRequest httpRequest) {
    ...
    found.page().content().stream().map(content ->
    new Option(content.id(), content.name())).toList()));
    }
    }
