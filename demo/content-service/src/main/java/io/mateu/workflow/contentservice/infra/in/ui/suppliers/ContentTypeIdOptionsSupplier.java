...
public class ContentTypeIdOptionsSupplier implements ForeignKeyOptionsSupplier {
...
public ListingData<Option> search(String searchText, Pageable pageable, HttpRequest httpRequest) {
    ...
    found.page().content().stream().map(contenttype ->
    new Option(contenttype.id(), contenttype.name())).toList()));
    }
    }
