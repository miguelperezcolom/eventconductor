...
public class AssetVersionIdOptionsSupplier implements ForeignKeyOptionsSupplier {
...
public ListingData<Option> search(String searchText, Pageable pageable, HttpRequest httpRequest) {
    ...
    found.page().content().stream().map(assetversion ->
    new Option(assetversion.id(), assetversion.name())).toList()));
    }
    }
