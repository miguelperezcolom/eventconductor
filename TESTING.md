...
| DIST-12 | **An unprocessable event does not stall the traffic around it.** Real traffic is driven with a report for a step execution the engine has never heard of mixed into it, and the real processes still finish. It matters because a poll batch now shares transactions: the scope has to be one process, not one batch. |
