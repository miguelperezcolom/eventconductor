...
/** Reactivates a disabled workflow definition, moving it from {@code DISABLED} back to {@code ACTIVE}. */
@Service
    ...
    public void handle(String workflowDefinitionId) {
        ...
        if (definition.status() != WorkflowDefinitionStatus.DISABLED) {
            throw new IllegalStateException(
                    "Only a DISABLED workflow can be reactivated (was " + definition.status() + ")");
        }
