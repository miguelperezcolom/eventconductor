---
name: eventconductor
description: Design and run business processes with EventConductor — the Java/Spring event-driven workflow & saga orchestration engine (io.mateu.workflow). Use when defining a workflow (JSON/YAML steps), implementing workers, starting/cancelling/querying processes, adding retries/timeouts/compensation (sagas), human USER_TASK forms, or child (PROCESS) workflows. Triggers on workflow-engine, WorkflowDefinition, ProcessUpstreamEventUseCase, UpdateStepExecutionUseCase, EmbeddedTaskExecutor, TaskExecutionRequested, ACTION/USER_TASK/FORK/JOIN steps, workflow.mode, workflow.persistence.
---
...
A worker for a step (embedded mode):
```java
@Bean
